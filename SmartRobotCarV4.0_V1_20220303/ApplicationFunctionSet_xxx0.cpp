...
int Find_Closest_Angle_Scan_2(int starting_angle, int number_arc_steps, int step_size)
{
  ...
  if (angle != -1)
    ...
    AppServo.DeviceDriverSet_Servo_control(90); // If nothing found just face forward
  
  return angle;
}


int Find_Closest_Angle_Scan(int starting_angle, int arc_size, int granularity)
{
    static uint16_t ULTRASONIC_Get = 0;
  int closest = 100; //Default to 100cm range
  int angle = -1; // -1 means nothing found within 100cm in the arc
  ...
  // Reset to the center
  AppServo.DeviceDriverSet_Servo_control(starting_angle);
...
uint8_t ApplicationFunctionSet::ApplicationFunctionSet_Follow3(uint8_t start)
{
  return Find_Closest_Angle_Scan(start,10,5);  
}
